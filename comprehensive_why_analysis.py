...
executed_long = [t for t in executed if t.get("signal_ctx", {}).get("side", "").upper() in ["LONG", "long"]]
executed_short = [t for t in executed if t.get("signal_ctx", {}).get("side", "").upper() == "SHORT"]

# Analyze blocked LONG trades
blocked_long = [t for t in blocked if t.get("signal_ctx", {}).get("side", "").upper() in ["LONG", "long"]]
blocked_short = [t for t in blocked if t.get("signal_ctx", {}).get("side", "").upper() == "SHORT"]

# Analyze counterfactual LONG (what blocked LONG trades would have done)
counterfactual_long = [t for t in counterfactual if t.get("side", "").upper() in ["LONG", "long"]]
counterfactual_short = [t for t in counterfactual if t.get("side", "").upper() == "SHORT"]
...
for trade in executed_long:
    ofi = trade.get("signal_ctx", {}).get("ofi", 0)
...
for trade in blocked_long:
    ofi = trade.get("signal_ctx", {}).get("ofi") or trade.get("intelligence", {}).get("ofi", 0)
    if isinstance(ofi, (int, float)):
...
counterfactual_long_winners = [t for t in counterfactual_long if t.get("would_have_won", False)]
counterfactual_long_losers = [t for t in counterfactual_long if not t.get("would_have_won", False)]
...
counterfactual_short_winners = [t for t in counterfactual_short if t.get("would_have_won", False)]
counterfactual_short_losers = [t for t in counterfactual_short if not t.get("would_have_won", False)]
...
if counterfactual:
    cf_winners = [t for t in counterfactual if t.get("would_have_won", False)]
    cf_losers = [t for t in counterfactual if not t.get("would_have_won", False)]
    ...
    # By direction
    cf_long_winners = [t for t in cf_winners if t.get("side", "").upper() in ["LONG", "long"]]
    cf_long_losers = [t for t in cf_losers if t.get("side", "").upper() in ["LONG", "long"]]
    cf_short_winners = [t for t in cf_winners if t.get("side", "").upper() == "SHORT"]
    cf_short_losers = [t for t in cf_losers if t.get("side", "").upper() == "SHORT"]
...
if missed:
    missed_long = [t for t in missed if t.get("side", "").upper() in ["LONG", "long"]]
    missed_short = [t for t in missed if t.get("side", "").upper() == "SHORT"]
...
if missed:
    missed_long_count = len([t for t in missed if t.get("side", "").upper() in ["LONG", "long"]])
