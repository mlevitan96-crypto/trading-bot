...
class HoldTimeEnforcer:
    ...
    def record_entry(self, position_id: str, symbol: str, side: str, entry_ts, position_data: Dict = None) -> Dict:
        ...
        # [BIG ALPHA PHASE 3 & 4] Force hold for TRUE TREND positions - extend based on Max Pain and Liquidation clusters
        if is_true_trend:
            ...
            # [BIG ALPHA PHASE 4] Check if moving toward Liquidation cluster (90min extension)
            extended_to_90min = False
            if entry_price > 0:
                try:
                    from src.intent_intelligence_guards import check_moving_toward_liquidation_cluster
                    ...
                    if current_price > 0:
                        is_moving_toward, cluster_info = check_moving_toward_liquidation_cluster(
                            symbol, current_price, entry_price, side.upper()
                        )
                        if is_moving_toward and cluster_info:
                            min_hold = 90 * 60  # 90 minutes = 5400 seconds (extended for liquidation magnetization)
                            cluster_price = cluster_info.get("price", 0)
                            _log(f"🔒 [TRUE-TREND-LIQ-MAGNET] Force-hold extended to 90min for {symbol} {side}: Moving toward liquidation cluster at ${cluster_price:.2f}")
                            extended_to_90min = True
                except Exception as e:
                    _log(f"⚠️ [TRUE-TREND] Error checking liquidation cluster: {e}")
            
            # [BIG ALPHA PHASE 3] If not extended to 90min, check Max Pain distance
            if not extended_to_90min:
